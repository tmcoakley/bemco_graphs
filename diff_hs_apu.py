...
def graphygraphtwo(apudata, hsdata,  error = False, title = False, num = False): 
    ...
    # Plot filtered data
    # ax.plot(list(apudata.keys()), apu_vals, '#9C4A6F', label = 'APU' )
    #  ax.plot(list(hsdata.keys()), hs_vals, "#688E4E",  label = 'Hot Swap' )
    ax.plot(list(hsdata.keys())[0:1820], apu_vals - hs_vals[0:1820], 'k', label = 'Difference')
    ax.plot()
